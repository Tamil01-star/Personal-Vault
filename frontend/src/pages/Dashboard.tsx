...
export const Dashboard: React.FC<DashboardProps> = ({ setCurrentView }) => {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      {/* Top Welcome Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-card to-card/50 border border-border p-6 rounded-2xl shadow-sm">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-foreground">
            {getGreeting()}, <span className="text-primary uppercase">{user?.username}</span>
          </h2>
        ...
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full bg-background border border-border focus:border-primary focus:ring-1 focus:ring-primary rounded-xl py-2 pl-9 pr-4 text-xs placeholder-muted-foreground focus:outline-none transition-all"
          />
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {searchResults.passwords.map((item) => (
                <div
                  ...
                  className="p-3 bg-muted/40 hover:bg-accent border border-border rounded-xl cursor-pointer flex justify-between items-center transition-colors"
                >
                  <div>
        ...
        {cards.map((card) => {
          ...
          return (
            <div
              ...
              className="bg-card border border-border hover:border-primary/40 p-5 rounded-2xl cursor-pointer hover:shadow-md transition-all duration-200 group relative overflow-hidden"
            >
              <div className="flex justify-between items-start">
                ...
                {loading ? (
                  <span className="w-6 h-6 rounded bg-muted animate-pulse" />
                ) : (
                  <span className="text-2xl font-bold tracking-tight text-foreground">{card.count}</span>
                )}
              </div>
              <h3 className="text-xs font-semibold text-muted-foreground mt-4 group-hover:text-primary transition-colors">
                {card.label}
              </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl shadow-sm p-6 lg:col-span-2 flex flex-col justify-between">
          <div>
              ...
              <div className="divide-y divide-border/60">
                {recentActivities.map((act, index) => (
                  <div
                    ...
                    className="flex items-start gap-4 py-3.5 hover:bg-muted/20 px-2 rounded-xl cursor-pointer transition-colors group"
                  >
        ...
        <div className="bg-card border border-border rounded-2xl shadow-sm p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            <button
              ...
              className="flex items-center justify-between p-3.5 rounded-xl border border-border bg-muted/20 hover:bg-accent text-left transition-colors group"
            >
